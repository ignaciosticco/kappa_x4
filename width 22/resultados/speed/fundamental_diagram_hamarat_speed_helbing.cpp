...
void escribir(vector<double> &vector_time,vector<int> &vector_N, vector<double> &vector_density,vector<double> &vector_speed_helbing_x,int n_samples,int density);


int main(int argc, char const *argv[]){
	for (int density_raw = 1; density_raw < 10; ++density_raw){
		char buffer [50];
  		sprintf (buffer, "configuraciones_density_%d_vd1_pasillo", density_raw);
	
		int n_samples = 0;
		vector<double> vector_time;
		vector<int> vector_N;
		vector<double> vector_density;
		vector<double> vector_speed_helbing_x;
		string archivoDatos;
		...
		while(fileIn.good()){
			...
			speed_helbing_x = vector_flujo[0]/density;
			vector_speed_helbing_x.push_back(speed_helbing_x); 
			vector_density.push_back(density);	
			n_samples++;

		}
		fileIn.close();
		escribir(vector_time,vector_N,vector_density,vector_speed_helbing_x,n_samples,density_raw);
	}
...
void escribir(vector<double> &vector_time,vector<int> &vector_N, vector<double> &vector_density,vector<double> &vector_speed_helbing_x,int n_samples,int density){
	...
	FILE *fp;
	char buffer [50];
	sprintf (buffer, "density_%d_speedhelbing_vd1_pasillo.txt", density); 
	fp=fopen(buffer,"w");
	for (int i = 0; i < n_samples; ++i){
		fprintf(fp,"%.2f %i %.3f %.3f\n",vector_time[i],vector_N[i],vector_density[i],vector_speed_helbing_x[i]);
	}
	fclose(fp);
}
