...
int main(int argc, char const *argv[]){
	for (int density_raw = 1; density_raw < 11; ++density_raw){
		char buffer [50];
  		sprintf (buffer, "config_density%d_width22", density_raw);
...
void escribir(vector<double> &vector_time,vector<int> &vector_N, vector<double> &vector_density,vector<double> &vector_flow_x,int n_samples,int density){
	...
	FILE *fp;
	char buffer [50];
	sprintf (buffer, "density_flow_%d_vd1_pasillo22m.txt", density); 
	fp=fopen(buffer,"w");
